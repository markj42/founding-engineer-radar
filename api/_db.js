...
export function makeDb(env = process.env) {
  ...
  return {
    listingsByKind: async (kind) => {
      const { data, error } = await supabase.from('radar_listings')
        .select('*').eq('kind', kind).order('seen_at', { ascending: false }).limit(100);
      if (error) throw new Error(error.message);
      return data ?? [];
    },
    matchesFor: async (companyHiId) => {
      const { data, error } = await supabase.from('radar_matches')
    ...
    lastOkRun: async () => {
      const { data, error } = await supabase.from('radar_scan_runs')
